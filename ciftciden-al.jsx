...
const G = {
  ...
  red:"#e05252", orange:"#e8952a", blue:"#3a7bd5", gold:"#d4a017"
};
...
/* ─── DATA ───────────────────────────────────────────────────────── */
const PRODUCTS = [
  {id:1,name:"Domates",emoji:"🍅",farmer:"Ahmet Usta",fid:1,loc:"Antalya",price:8,unit:"kg",cat:"Sebze",org:true,rating:4.8,reviews:34},
  {id:2,name:"Yeşil Biber",emoji:"🫑",farmer:"Fatma Hanım",fid:2,loc:"Mersin",price:12,unit:"kg",cat:"Sebze",org:false,rating:4.5,reviews:21},
  {id:3,name:"Patlıcan",emoji:"🍆",farmer:"Mehmet Bey",fid:3,loc:"Adana",price:10,unit:"kg",cat:"Sebze",org:true,rating:4.7,reviews:18},
  {id:4,name:"Elma",emoji:"🍎",farmer:"Zeynep Hanım",fid:4,loc:"Isparta",price:15,unit:"kg",cat:"Meyve",org:true,rating:5.0,reviews:52},
  {id:5,name:"Portakal",emoji:"🍊",farmer:"Ali Usta",fid:5,loc:"Adana",price:9,unit:"kg",cat:"Meyve",org:false,rating:4.3,reviews:29},
  {id:6,name:"Üzüm",emoji:"🍇",farmer:"Hüseyin Bey",fid:6,loc:"Manisa",price:18,unit:"kg",cat:"Meyve",org:true,rating:4.9,reviews:41},
  {id:7,name:"Marul",emoji:"🥬",farmer:"Fatma Hanım",fid:2,loc:"Mersin",price:5,unit:"adet",cat:"Sebze",org:true,rating:4.6,reviews:15},
  {id:8,name:"Bal",emoji:"🍯",farmer:"Mustafa Bey",fid:7,loc:"Karadeniz",price:120,unit:"kg",cat:"Diğer",org:true,rating:5.0,reviews:67},
  {id:9,name:"Kabak",emoji:"🥒",farmer:"Mehmet Bey",fid:3,loc:"Adana",price:7,unit:"kg",cat:"Sebze",org:true,rating:4.7,reviews:9},
  {id:10,name:"Çilek",emoji:"🍓",farmer:"Zeynep Hanım",fid:4,loc:"Isparta",price:25,unit:"kg",cat:"Meyve",org:true,rating:4.9,reviews:38},
];

const FARMERS_DB = {
  1:{name:"Ahmet Usta",emoji:"👨‍🌾",loc:"Antalya",about:"30 yıldır domates yetiştiriyorum. Tohumlarımı kendim saklıyor, hiç kimyasal kullanmıyorum. Ürünlerimi sabah erkenden topluyorum, aynı gün yola çıkıyor.",rating:4.8,sales:1240,since:"1994",tags:["Organik","Sertifikalı","Hızlı Teslimat"],verified:true,reviews:[{user:"Ayşe K.",stars:5,text:"Domatesler gerçekten tarladan gelmiş gibi, süpermarket domatesiyle kıyaslanamaz!"},{user:"Can M.",stars:5,text:"Paketleme çok özenli, hiçbiri ezilmemiş. Kesinlikle tavsiye ederim."}]},
  2:{name:"Fatma Hanım",emoji:"👩‍🌾",loc:"Mersin",about:"Aile çiftliğimizde 3 nesildir sebze yetiştiriyoruz. Biber ve marulda uzmanız.",rating:4.6,sales:892,since:"2003",tags:["Aile Çiftliği","Taze","Uygun Fiyat"],verified:true,reviews:[{user:"Mert Y.",stars:4,text:"Biberler çok tazeydi, teşekkürler."},{user:"Elif T.",stars:5,text:"Marullar harika, haftaya da sipariş vereceğim!"}]},
  3:{name:"Mehmet Bey",emoji:"🧑‍🌾",loc:"Adana",about:"Patlıcan ve kabak konusunda uzmanım. Adana toprağı çok verimli.",rating:4.7,sales:654,since:"2010",tags:["Organik","Yerel Tohumlar"],verified:false,reviews:[{user:"Hande A.",stars:5,text:"Patlıcanlar etli ve lezzetliydi!"},{user:"Burak S.",stars:4,text:"Hızlı teslimat, taze ürün."}]},
  4:{name:"Zeynep Hanım",emoji:"👩‍🌾",loc:"Isparta",about:"Isparta'nın meşhur elmaları ve çileklerini yetiştiriyorum. Her birini elle topluyoruz.",rating:4.9,sales:2100,since:"1998",tags:["Organik","El Hasadı","Sertifikalı"],verified:true,reviews:[{user:"Selin K.",stars:5,text:"Elmalar müthiş, marketten alamazsınız böylesini!"},{user:"Ozan B.",stars:5,text:"Çilekler fevkalade, her hafta sipariş veriyorum."}]},
};

const FARMERS_PENDING = [
  {id:8,name:"Kadir Bey",emoji:"👨‍🌾",loc:"Konya",products:"Buğday, Nohut",phone:"0532 111 2233",status:"pending"},
  {id:9,name:"Selma Hanım",emoji:"👩‍🌾",loc:"Bursa",products:"Şeftali, Erik",phone:"0544 333 4455",status:"pending"},
];

const CONVOS_INIT = {
  1:[{from:"them",text:"Merhaba! Domates siparişinizi aldım 🍅"},{from:"me",text:"Teşekkürler! Ne zaman hazır olur?"},{from:"them",text:"Yarın sabah kargoya veriyorum, takip kodunu WhatsApp'tan göndereceğim 🚚"}],
  2:[{from:"me",text:"Biber organik mi?"},{from:"them",text:"Evet, organik sertifikalıyız 🌿 Doğrulamalı çiftçiyiz."}],
  3:[{from:"me",text:"5 kg patlıcan ne kadar?"},{from:"them",text:"5 kg için 45₺ yapabilirim, kargo dahil mi istersiniz?"}],
};

const MSGS_INIT = [
  {id:1,name:"Ahmet Usta",emoji:"👨‍🌾",prev:"Yarın sabah kargoya veriyorum",time:"09:12",unread:1},
  {id:2,name:"Fatma Hanım",emoji:"👩‍🌾",prev:"Evet, organik sertifikalıyız",time:"Dün",unread:0},
  {id:3,name:"Mehmet Bey",emoji:"🧑‍🌾",prev:"5 kg için 45₺ yapabilirim",time:"Pzt",unread:2},
];

const ORDERS_INIT = [
  {id:"#CF2041",items:["🍅 Domates 3kg","🥬 Marul 2 adet"],farmer:"Ahmet Usta",total:49,status:2,date:"3 Mayıs",kargo:"Yurtiçi Kargo",kod:"YK38291047",addr:"Seyhan, Adana"},
  {id:"#CF2038",items:["🍎 Elma 5kg"],farmer:"Zeynep Hanım",total:100,status:3,date:"28 Nisan",kargo:"Ücretsiz (Öz Kurye)",kod:"OZ10284",addr:"Çukurova, Adana"},
];

const SL = ["Sipariş Alındı","Çiftçi Onayladı","Kargoda","Teslim Edildi"];
const SC = ["s0","s1","s2","s3"];

function Stars({n}){return <span className="stars">{"★".repeat(Math.round(n))}{"☆".repeat(5-Math.round(n))} {n?.toFixed(1)}</span>}
...
export default function App(){
  ...
  useEffect(()=>{ endRef.current?.scrollIntoView({behavior:"smooth"}); }, [convos, activeChat]);
  ...
  const handleAuth = () => {
    if(authMode==="register" && !form.name){ showToast("İsim girin!"); return; }
    if(!form.email || !form.password){ showToast("Tüm alanları doldurun!"); return; }
    const u = {
      name: form.name||"Kullanıcı", email: form.email,
      role: authRole, emoji: authRole==="buyer"?"🛒":authRole==="farmer"?"👨‍🌾":"⚙️",
      phone: form.phone||"—", city: form.city||"Adana",
      verified: authRole==="farmer" ? false : true
    };
    setUser(u); setRole(authRole);
    setTab(authRole==="farmer"?"panel":authRole==="admin"?"admin":"market");
    setScreen("app");
  ...
  const sendMsg = () => {
    ...
    setConvos(prev=>({...prev,[activeChat]:[...(prev[activeChat]||[]),{from:"me",text:chatMsg}]}));
    setChatMsg("");
    setTimeout(()=>setConvos(prev=>({...prev,[activeChat]:[...prev[activeChat],{from:"them",text:"Mesajınız için teşekkürler! 🌿"}]})), 1000);
  };

  const placeOrder = () => {
    if(!addr.street || !addr.city){ showToast("Lütfen teslimat adresini doldurun!"); return; }
    const kargoLabel = kargoType==="oz" ? "Öz Kurye (Kısa Mesafe)" : "Yurtiçi Kargo";
    ...
    const o = {
      ...
      status: 0, date: "Bugün",
      kargo: kargoLabel, kod, addr:`${addr.district}, ${addr.city}`
    };
  ...
  const approveFarmer = (id) => {
    setPendingFarmers(prev=>prev.map(f=>f.id===id?{...f,status:"approved"}:f));
  ...
  if(screen==="splash") return (
    ...
    <div className="splash">
      ...
      <div className="role-cards">
        <div className="role-card" onClick={()=>{setAuthRole("buyer");setScreen("auth");}}>
          <span className="icon">🛒</span>
          <h3>Alıcıyım</h3>
          <p>Taze ürünü direkt çiftçiden, uygun fiyata al</p>
        </div>
        <div className="role-card" onClick={()=>{setAuthRole("farmer");setScreen("auth");}}>
          <span className="icon">👨‍🌾</span>
          <h3>Çiftçiyim</h3>
          <p>Ürünlerini listele, aracısız daha fazla kazan</p>
        </div>
      </div>
      <span className="admin-link" onClick={()=>{setAuthRole("admin");setScreen("auth");}}>Yönetici Girişi</span>
    </div></>
  );
  ...
  if(screen==="auth") return (
    ...
    <div className="auth-wrap"><div className="auth-card">
      ...
      {authRole !== "admin" && <div className="auth-tabs">
        <button className={`auth-tab ${authMode==="login"?"active":""}`} onClick={()=>setAuthMode("login")}>Giriş Yap</button>
        <button className={`auth-tab ${authMode==="register"?"active":""}`} onClick={()=>setAuthMode("register")}>Kayıt Ol</button>
      </div>}
      {authMode==="register" && authRole!=="admin" && <>
        ...
        {authRole==="farmer" && <div className="note-box">📋 Çiftçi kaydında bilgileriniz admin tarafından doğrulanacak. Onaylandıktan sonra ürün ekleyebilirsiniz.</div>}
      ...
      <div className="field"><label>Şifre</label><input className="inp" placeholder="••••••••" type="password" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))}/></div>
      <button className="auth-btn" onClick={handleAuth}>
        {authRole==="admin"?"Yönetici Olarak Giriş →":authMode==="login"?"Giriş Yap →":"Kayıt Ol →"}
      </button>
      <span className="auth-back" onClick={()=>setScreen("splash")}>← Geri dön</span>
    </div></div></>
  );
  ...
  return (
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      {/* NAV */}
      <nav className="nav">
        <div className="nav-logo" onClick={()=>{setTab(role==="buyer"?"market":role==="farmer"?"panel":"admin");setFarmerView(null);}}>🌿 Çiftçiden <em>Al</em></div>
        <div className="nav-center">
          <span className="nbadge">{role==="buyer"?"Alıcı":role==="farmer"?"Çiftçi":"Admin"}</span>
          {role==="farmer" && user?.verified && <span className="vbadge">✓ Doğrulandı</span>}
          <span className="nav-user">{user?.name}</span>
        </div>
        <div className="nav-actions">
          {role==="buyer" && <button className="nbtn" onClick={()=>setTab("cart")}>🧺{cartCount>0&&<span className="bdg">{cartCount}</span>}</button>}
          <button className="nbtn" onClick={()=>{setScreen("splash");setUser(null);setRole(null);setCart([]);setFarmerView(null);}}>Çıkış</button>
        </div>
      </nav>

      {/* TABS */}
      <div className="tabs">
        {allTabs.map(t=><button key={t.key} className={`tab ${tab===t.key?"active":""}`} onClick={()=>{setTab(t.key);setActiveChat(null);setFarmerView(null);}}>{t.label}</button>)}
      </div>
      ...
      <div className="main">
        ...
        {tab==="market" && !farmerView && <>
          <div className="sh2"><h2>Taze Ürünler</h2><p>Çiftçilerden direkt, aracısız ve taze</p></div>
          ...
          <div className="pills">
            {["Tümü","Sebze","Meyve","Organik","Diğer"].map(f=><button key={f} className={`pill ${filter===f?"active":""}`} onClick={()=>setFilter(f)}>{f}</button>)}
          </div>
          <div className="pgrid">
            {filtered.map(p=>(
              <div key={p.id} className="pcard">
                <div className="pimg">
                  ...
                  {FARMERS_DB[p.fid]?.verified && <span className="vrf-dot">✓</span>}
                </div>
                <div className="pinfo">
                  <Stars n={p.rating}/>
                  <div className="pname">{p.name}</div>
                  <div className="pfarmer" onClick={()=>{if(FARMERS_DB[p.fid])setFarmerView(p.fid);}}>👨‍🌾 {p.farmer}</div>
                  <div className="ploc">📍 {p.loc}</div>
                  <div className="pfoot">
                    <div><span className="pp">{p.price}₺</span><span className="pu"> /{p.unit}</span></div>
                    {role!=="admin" && <button className="addbtn" onClick={()=>addToCart(p)}>+ Ekle</button>}
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* ── FARMER PROFILE ── */}
        {tab==="market" && farmerView && (()=>{
          const f = FARMERS_DB[farmerView]; if(!f) return null;
          return <>
            <button className="back-btn" onClick={()=>setFarmerView(null)}>← Geri</button>
            <div className="fp-wrap">
              <div className="fp-hdr">
                <div className="fp-av">{f.emoji}</div>
                <div>
                  <div className="fp-name">{f.name} {f.verified && <span style={{fontSize:".7rem",background:"var(--gold)",color:"#fff",padding:".12rem .45rem",borderRadius:"6px",marginLeft:".4rem"}}>✓ Doğrulandı</span>}</div>
                  <div style={{opacity:.82,fontSize:".88rem",margin:".2rem 0"}}>📍 {f.loc} · {f.since}'den beri</div>
                  <div className="fp-tags">{f.tags.map(t=><span key={t} className="fp-tag">{t}</span>)}</div>
                </div>
              </div>
              <div className="fp-body">
                <div className="fp-stats">
                  <div className="fp-stat"><div className="fp-sn">{f.rating}</div><div className="fp-sl">⭐ Puan</div></div>
                  <div className="fp-stat"><div className="fp-sn">{f.sales}</div><div className="fp-sl">Toplam Satış</div></div>
                  <div className="fp-stat"><div className="fp-sn">{new Date().getFullYear()-parseInt(f.since)}</div><div className="fp-sl">Yıl Deneyim</div></div>
                </div>
                <div className="fp-about">{f.about}</div>
                <h4 style={{fontFamily:"'Playfair Display',serif",color:"var(--g2)",marginBottom:".7rem"}}>Müşteri Yorumları</h4>
                <div className="rev-list">
                  {f.reviews.map((r,i)=>(
                    <div key={i} className="rev">
                      <div className="rev-top"><span className="rev-user">👤 {r.user}</span><span className="stars">{"★".repeat(r.stars)}</span></div>
                      <div className="rev-text">{r.text}</div>
                    </div>
                  ))}
                </div>
                {/* Yorum ekle */}
                <div style={{background:"var(--gm)",borderRadius:"12px",padding:"1rem",marginTop:".8rem"}}>
                  <div style={{fontWeight:600,fontSize:".88rem",marginBottom:".6rem"}}>Yorum Yaz</div>
                  <div style={{display:"flex",gap:".3rem",marginBottom:".6rem"}}>
                    {[1,2,3,4,5].map(s=>(
                      <span key={s} style={{fontSize:"1.4rem",cursor:"pointer",color:(ratingInput[farmerView]||0)>=s?"var(--orange)":"var(--gp)"}} onClick={()=>setRatingInput(r=>({...r,[farmerView]:s}))}>★</span>
                    ))}
                  </div>
                  <input className="inp" placeholder="Deneyiminizi paylaşın..." value={reviewText[farmerView]||""} onChange={e=>setReviewText(r=>({...r,[farmerView]:e.target.value}))} style={{marginBottom:".6rem"}}/>
                  <button className="btn-green" style={{width:"100%"}} onClick={()=>{if(reviewText[farmerView]){showToast("✅ Yorumunuz eklendi, teşekkürler!");setReviewText(r=>({...r,[farmerView]:""}))}else showToast("Lütfen yorum yazın!");}}>Yorum Gönder</button>
                </div>
                <button className="subbtn" style={{marginTop:"1rem",width:"100%"}} onClick={()=>{setActiveChat(1);setTab("messages");}}>💬 Çiftçiye Mesaj Gönder</button>
              </div>
            </div>
          </>;
        })()}
        ...
        {/* ── MAP ── */}
        {tab==="map" && <>
          <div className="sh2"><h2>Çiftçi Haritası</h2><p>Yakınındaki çiftçileri keşfet</p></div>
          <div className="map-box">
            <div className="map-hdr"><h3>🗺️ Türkiye Geneli</h3><span style={{fontSize:".8rem",color:"var(--tl)"}}>7 aktif çiftçi</span></div>
            <div className="map-vis">
              <div className="map-grid"/>
              {[
                {top:"22%",left:"28%",n:"Ahmet Usta",fid:1},{top:"48%",left:"62%",n:"Fatma H.",fid:2},
                {top:"32%",left:"72%",n:"Mehmet B.",fid:3},{top:"18%",left:"45%",n:"Zeynep H.",fid:4},
                {top:"15%",left:"55%",n:"Ali Usta",fid:null},{top:"60%",left:"32%",n:"Hüseyin B.",fid:null},
                {top:"12%",left:"68%",n:"Mustafa B.",fid:null},
              ].map((p,i)=>(
                <div key={i} className="mpin" style={{top:p.top,left:p.left}} onClick={()=>{if(p.fid){setFarmerView(p.fid);setTab("market");}}}>
                  <span className="mpin-i">📍</span><div className="mpin-l">{p.n}</div>
                </div>
              ))}
            </div>
            <div className="map-leg">
              ...
              <div className="leg-i" style={{color:"var(--gold)"}}>✓ Doğrulanmış</div>
            </div>
          </div>
          <div style={{fontSize:".84rem",color:"var(--tl)",textAlign:"center"}}>İkona tıklayarak çiftçi profilini görüntüle</div>
        </>}
        ...
        {tab==="messages" && <>
          {!activeChat ? <>
            ...
            <div className="msg-list">
              {MSGS_INIT.map(m=>(
                <div key={m.id} className="msg-item" onClick={()=>setActiveChat(m.id)}>
                  <div className="mav">{m.emoji}</div>
                  <div className="mif"><div className="mn">{m.name}</div><div className="mp">{m.prev}</div></div>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:".3rem"}}>
                    <span className="mt">{m.time}</span>
                    {m.unread>0 && <span className="mu">{m.unread}</span>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="chat-hdr">
              <button className="cbk" onClick={()=>setActiveChat(null)}>←</button>
              <span style={{fontSize:"1.3rem"}}>{MSGS_INIT.find(m=>m.id===activeChat)?.emoji}</span>
              <div>
                <div style={{fontWeight:700}}>{MSGS_INIT.find(m=>m.id===activeChat)?.name}</div>
                <div style={{fontSize:".75rem",opacity:.8}}>🟢 Çevrimiçi</div>
              </div>
              <button className="nbtn" style={{marginLeft:"auto"}} onClick={()=>{const n=MSGS_INIT.find(m=>m.id===activeChat)?.name;window.open(`https://wa.me/?text=Merhaba ${n}, Çiftçiden Al uygulamasından yazıyorum.`,'_blank');}}>📱 WhatsApp</button>
            </div>
            <div className="chat-msgs">
              {(convos[activeChat]||[]).map((msg,i)=><div key={i} className={`bbl ${msg.from}`}>{msg.text}</div>)}
              <div ref={endRef}/>
            </div>
            <div className="chat-inp-row">
              <input className="ci" placeholder="Mesajınızı yazın..." value={chatMsg} onChange={e=>setChatMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg()}/>
        ...
        {/* ── ORDERS (Alıcı) ── */}
        {tab==="orders" && <>
          <div className="sh2"><h2>Siparişlerim</h2><p>Tüm siparişlerini takip et</p></div>
          <div className="orders-list">
            {orders.map(o=>{
              ...
              return <div key={o.id} className="ocard">
                <div className="otop">
                  <div>
                    <div className="oid">{o.id} · {o.date}</div>
                    <div style={{fontSize:".82rem",color:"var(--tl)",marginTop:".18rem"}}>👨‍🌾 {o.farmer}</div>
                  </div>
                  <span className={`ostatus ${SC[o.status]}`}>{SL[o.status]}</span>
                </div>
                <div className="oitems">{o.items.map((it,i)=><span key={i} className="otag">{it}</span>)}</div>
                <div className="prog-bar"><div className="prog-fill" style={{width:`${pct}%`}}/></div>
                <div className="prog-steps">{SL.map((l,i)=><span key={i} className={i<=o.status?"dn":""}>{l}</span>)}</div>
                <div className="ofoot">
                  <div>
                    <div className="ototal">{o.total}₺</div>
                    <div className="kargo-info">🚚 {o.kargo}</div>
                    {o.status >= 2 && <div style={{marginTop:".4rem",display:"flex",alignItems:"center",gap:".5rem"}}>
                      <span style={{fontSize:".75rem",color:"var(--tl)"}}>Takip Kodu:</span>
                      <span className="kargo-kod">{o.kod}</span>
                    </div>}
                    <div style={{fontSize:".73rem",color:"var(--tl)",marginTop:".2rem"}}>📍 {o.addr}</div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:".4rem"}}>
                    <button className="nbtn" style={{background:"var(--gm)",color:"var(--g2)"}} onClick={()=>{setActiveChat(1);setTab("messages");}}>💬 Çiftçiye Sor</button>
                    {o.status===3 && <button className="nbtn" style={{background:"var(--g4)",color:"var(--g2)"}} onClick={()=>showToast("Tekrar sipariş sepete eklendi!")}>🔄 Tekrar Sipariş</button>}
                  </div>
                </div>
              </div>;
            })}
        ...
        {tab==="cart" && <>
          ...
          {cart.length===0 ? <div className="cart-empty">
            ...
            <p style={{fontSize:".83rem",marginTop:".45rem",color:"var(--tl)"}}>Marketten ürün ekleyin</p>
            ...
            <div className="cargo-box">
              ...
              {cartTotal >= 300 && <div style={{background:"var(--g4)",color:"var(--g2)",borderRadius:"10px",padding:".6rem .9rem",fontSize:".85rem",fontWeight:600,marginBottom:".75rem"}}>🎉 300₺ üzeri siparişte kargo ücretsiz!</div>}
              <div className="cargo-opts">
                <div className={`cargo-opt ${kargoType==="oz"?"sel":""}`} onClick={()=>setKargoType("oz")}>
                  ...
                  <div className="cprice">{cartTotal>=300?<>Ücretsiz <span className="free-badge">ÜCRETSİZ</span></>:"20₺"}</div>
                </div>
                <div className={`cargo-opt ${kargoType==="yurtici"?"sel":""}`} onClick={()=>setKargoType("yurtici")}>
                  <h5>📦 Yurtiçi Kargo</h5>
                  <p>Uzak mesafe, 1-3 iş günü teslim.</p>
                  <div className="cprice">{cartTotal>=300?<>Ücretsiz <span className="free-badge">ÜCRETSİZ</span></>:"30₺"}</div>
                </div>
              </div>
            </div>
            ...
            <div className="addr-box">
              ...
              <div className="addr-row">
                <input className="addr-inp" placeholder="Sokak / Cadde / Mahalle" value={addr.street} onChange={e=>setAddr(a=>({...a,street:e.target.value}))}/>
              </div>
              <div className="addr-row">
              ...
              <input className="addr-inp" placeholder="Kapı notu (opsiyonel)" value={addr.note} onChange={e=>setAddr(a=>({...a,note:e.target.value}))} style={{width:"100%"}}/>
            </div>

            {/* ÖZET */}
            <div className="cart-sum">
              <div className="sr"><span>Ara toplam</span><span>{cartTotal}₺</span></div>
              <div className="sr"><span>Aracı komisyonu</span><span style={{color:"var(--g5)"}}>0₺ 🎉</span></div>
              <div className="sr"><span>Kargo ({kargoType==="oz"?"Öz Kurye":"Yurtiçi"})</span><span>{kargoBedel===0?"Ücretsiz ✓":`${kargoBedel}₺`}</span></div>
              <div className="sdiv">
                <div className="sr big"><span>Toplam</span><span>{cartTotal+kargoBedel}₺</span></div>
              </div>
              <button className="ordbtn" onClick={placeOrder}>Sipariş Ver 🌿</button>
              <button className="wabtn" onClick={()=>window.open(`https://wa.me/905001234567?text=Merhaba! Çiftçiden Al uygulamasından sipariş vermek istiyorum. Toplam: ${cartTotal+kargoBedel}₺`,'_blank')}>
        ...
        {tab==="panel" && <div className="fpanel">
          <div className="sh2"><h2>Çiftçi Paneliniz</h2><p>Hoş geldin, {user?.name}! {user?.verified ? "✅ Hesabınız doğrulandı." : "⏳ Hesabınız onay bekliyor."}</p></div>
          {!user?.verified && <div className="note-box" style={{borderColor:"var(--orange)"}}>⏳ Hesabınız admin tarafından inceleniyor. Onaylandıktan sonra ürün ekleyebileceksiniz.</div>}
          <div className="pc"><h3>📊 Bu Ay Özet</h3>
            <div className="stat-row">
              <div className="stb"><div className="stn">12</div><div className="stl">Aktif Ürün</div></div>
              <div className="stb"><div className="stn">47</div><div className="stl">Sipariş</div></div>
              <div className="stb"><div className="stn">3.840₺</div><div className="stl">Kazanç</div></div>
            </div>
          </div>
          <div className="pc"><h3>➕ Yeni Ürün Ekle</h3>
            <div className="fc">
              ...
              <div className="fr">
                ...
                <input className="fi" placeholder="Stok miktarı" value={newProd.stock} onChange={e=>setNewProd(p=>({...p,stock:e.target.value}))}/>
              </div>
              <div className="fr">
              ...
              <div style={{background:"var(--gm)",borderRadius:"10px",padding:".8rem",fontSize:".8rem",color:"var(--tm)"}}>
                📸 Fotoğraf yükleme özelliği yakında eklenecek. Şimdilik emoji kullanabilirsiniz.
              </div>
              <button className="subbtn" onClick={()=>{
                if(newProd.name&&newProd.price){showToast(`${newProd.emoji} ${newProd.name} listelendi!`);setNewProd({name:"",price:"",unit:"kg",emoji:"🌱",cat:"Sebze",stock:""});}
                else showToast("Ürün adı ve fiyat girin!");
              }}>Ürünü Listele 🌿</button>
            </div>
          </div>
          <div className="pc"><h3>📦 Son Siparişler</h3>
            {[
              {name:"Domates 5kg",buyer:"Ayşe H.",st:"Bekliyor",price:"40₺",stc:"s0",kargo:"Öz Kurye"},
              {name:"Biber 3kg",buyer:"Can B.",st:"Onaylandı",price:"36₺",stc:"s1",kargo:"Yurtiçi"},
              {name:"Patlıcan 2kg",buyer:"Mert Y.",st:"Teslim edildi",price:"20₺",stc:"s3",kargo:"Yurtiçi"},
            ].map((o,i)=>(
              <div key={i} className="order-row">
                <div>
                  <div style={{fontWeight:600,fontSize:".88rem"}}>{o.name}</div>
                  <div style={{fontSize:".75rem",color:"var(--tl)"}}>👤 {o.buyer} · 🚚 {o.kargo}</div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:".55rem"}}>
                  <span className={`ostatus ${o.stc}`}>{o.st}</span>
                  <span style={{fontWeight:700,color:"var(--g2)"}}>{o.price}</span>
                  {o.st==="Bekliyor"&&<button className="cnfbtn" onClick={()=>showToast("✅ Sipariş onaylandı! Kargo kodu oluşturuldu.")}>Onayla</button>}
                </div>
              </div>
            ))}
          </div>
          <div className="pc"><h3>📱 WhatsApp Bildirimleri</h3>
            <p style={{fontSize:".86rem",color:"var(--tm)",marginBottom:".9rem",lineHeight:1.6}}>Yeni sipariş geldiğinde WhatsApp'tan anında bildirim al. Numaranı ekle, hiçbir siparişi kaçırma!</p>
            <div className="fr">
              <input className="fi" placeholder="WhatsApp numaranız" type="tel"/>
              <button className="subbtn" onClick={()=>showToast("📱 WhatsApp bildirimleri aktif edildi!")}>Aktif Et</button>
            </div>
          </div>
        </div>}
        ...
        {/* ── ADMIN DASHBOARD ── */}
        {tab==="admin" && <>
          <div className="sh2"><h2>Admin Paneli</h2><p>Platform genel durumu</p></div>
          <div className="admin-grid">
            <div className="admin-stat"><div className="n">36</div><div className="l">Toplam Kullanıcı</div></div>
            <div className="admin-stat"><div className="n">12</div><div className="l">Aktif Çiftçi</div></div>
            <div className="admin-stat"><div className="n">148</div><div className="l">Toplam Sipariş</div></div>
            <div className="admin-stat"><div className="n">2</div><div className="l">Onay Bekleyen</div></div>
            <div className="admin-stat"><div className="n">8.240₺</div><div className="l">Bu Ay Ciro</div></div>
            <div className="admin-stat"><div className="n">4.8⭐</div><div className="l">Ort. Memnuniyet</div></div>
          </div>
          {pendingFarmers.filter(f=>f.status==="pending").length > 0 && <>
            <div className="pc">
              <h3>⏳ Onay Bekleyen Çiftçiler</h3>
              <div className="farmer-list">
                {pendingFarmers.filter(f=>f.status==="pending").map(f=>(
                  <div key={f.id} className="farmer-row">
                    ...
                    <div className="farmer-row-info">
                      ...
                      <div className="farmer-row-detail">📍 {f.loc} · 📱 {f.phone}</div>
                      <div className="farmer-row-detail">🌿 {f.products}</div>
                    </div>
                    <div style={{display:"flex",gap:".4rem"}}>
                      <button className="appr-btn" onClick={()=>approveFarmer(f.id)}>✓ Onayla</button>
                      <button className="rej-btn" onClick={()=>rejectFarmer(f.id)}>✗ Reddet</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="farmer-list">
              {Object.values(FARMERS_DB).map((f,i)=>(
                <div key={i} className="farmer-row">
                  <div style={{fontSize:"2rem"}}>{f.emoji}</div>
                  <div className="farmer-row-info">
                    <div className="farmer-row-name">{f.name} {f.verified&&<span style={{color:"var(--gold)",fontSize:".75rem"}}>✓ Doğrulandı</span>}</div>
                    <div className="farmer-row-detail">📍 {f.loc} · ⭐ {f.rating} · {f.sales} satış</div>
                  </div>
                  <span className="approved">Aktif</span>
                </div>
              ))}
              {pendingFarmers.filter(f=>f.status==="approved").map(f=>(
                <div key={f.id} className="farmer-row">
                  <div style={{fontSize:"2rem"}}>{f.emoji}</div>
                  <div className="farmer-row-info">
                    <div className="farmer-row-name">{f.name} <span style={{color:"var(--g4)",fontSize:".75rem"}}>✓ Yeni Onaylandı</span></div>
                    <div className="farmer-row-detail">📍 {f.loc} · 🌿 {f.products}</div>
                  </div>
                  <span className="approved">Aktif</span>
                </div>
              ))}
        ...
        {/* ── ADMIN FARMERS ── */}
        {tab==="farmers" && <>
          <div className="sh2"><h2>Çiftçi Yönetimi</h2><p>Başvuruları incele, onayla veya reddet</p></div>
          <div className="pc">
            <h3>Tüm Başvurular</h3>
            <div className="farmer-list">
              {[...Object.values(FARMERS_DB), ...pendingFarmers].map((f,i)=>(
                <div key={i} className="farmer-row">
                  <div style={{fontSize:"2rem"}}>{f.emoji}</div>
                  <div className="farmer-row-info">
                    <div className="farmer-row-name">{f.name}</div>
                    <div className="farmer-row-detail">📍 {f.loc} · {f.verified||f.status==="approved"?"✅ Onaylı":"⏳ Bekliyor"}</div>
                  </div>
                  {(f.status==="pending") ? <div style={{display:"flex",gap:".4rem"}}>
                    <button className="appr-btn" onClick={()=>approveFarmer(f.id)}>✓ Onayla</button>
                    <button className="rej-btn" onClick={()=>rejectFarmer(f.id)}>✗</button>
                  </div> : <span className="approved">Aktif</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── ADMIN ORDERS ── */}
        {tab==="orders-admin" && <>
          <div className="sh2"><h2>Tüm Siparişler</h2><p>Platform genelindeki siparişler</p></div>
          <div className="orders-list">
            {orders.map(o=>(
              <div key={o.id} className="ocard">
                <div className="otop">
                  <div><div className="oid">{o.id} · {o.date}</div><div style={{fontSize:".8rem",color:"var(--tl)"}}>👨‍🌾 {o.farmer} · 📍 {o.addr}</div></div>
                  <span className={`ostatus ${SC[o.status]}`}>{SL[o.status]}</span>
                </div>
                <div className="oitems">{o.items.map((it,i)=><span key={i} className="otag">{it}</span>)}</div>
                <div className="ofoot">
                  <div>
                    <div className="ototal">{o.total}₺</div>
                    <div className="kargo-info">🚚 {o.kargo} · <span className="kargo-kod">{o.kod}</span></div>
                  </div>
                  {o.status < 3 && <button className="cnfbtn" onClick={()=>{setOrders(prev=>prev.map(ord=>ord.id===o.id?{...ord,status:Math.min(3,ord.status+1)}:ord));showToast("Durum güncellendi!");}}>Sonraki Adım →</button>}
                </div>
              </div>
            ))}
          </div>
        ...
        {tab==="profile" && <>
          ...
          <div className="profile-card">
            ...
            <div className="prrole">{role==="buyer"?"Alıcı Hesabı":role==="farmer"?"Çiftçi Hesabı":"Admin"}</div>
            <div className="info-list">
              <div className="info-row"><span>📧 E-posta</span><span>{user?.email}</span></div>
              <div className="info-row"><span>📱 Telefon</span><span>{user?.phone}</span></div>
              <div className="info-row"><span>📍 Şehir</span><span>{user?.city}</span></div>
              <div className="info-row"><span>👤 Hesap</span><span>{role==="buyer"?"Alıcı":role==="farmer"?"Çiftçi":"Admin"}</span></div>
              {role==="farmer" && <div className="info-row"><span>✅ Durum</span><span style={{color:user?.verified?"var(--g3)":"var(--orange)"}}>{user?.verified?"Doğrulandı":"Onay Bekliyor"}</span></div>}
              {role==="buyer" && <div className="info-row"><span>📦 Sipariş</span><span>{orders.length}</span></div>}
            </div>
            <button className="logout-btn" onClick={()=>{setScreen("splash");setUser(null);setRole(null);setCart([]);}}>Çıkış Yap</button>
          </div>
        </>}
